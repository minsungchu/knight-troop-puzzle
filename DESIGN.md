...
| 랭킹 보드 | **정사각만** — 5×5 ~ 12×12 여덟 종류 × 난이도 3종 = 24개 보드. 가로·세로가 다른 판은 순위 없음 |
...
| 인원 | 1(솔로) ~ 4명 |
| 규격/난이도 | 방장이 지정. 정사각만 순위가 잡히므로 직사각형 판은 랭킹 미등록 |
...
정사각 규격 + 힌트 0회면 솔로와 동일하게 `scores`에 등록됩니다 (`room_id` 기록).
...
- **가로·세로가 다른 판으로도 대전은 가능**합니다. 랭킹에만 안 올라갑니다

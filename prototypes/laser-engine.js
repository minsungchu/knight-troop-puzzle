...
/* ══════════════ 난이도 점수 ══════════════
   처음에는 솔버가 펼친 노드 수를 주 지표로 삼았다. A* 나 BFS 가 펼친 노드 수가 사람이
   느끼는 난이도와 상관이 높다는 리서치가 근거였다. 8판을 사람이 직접 풀어 재 보니
   그게 뒤집혔다 — 이 점수는 푼 시간과 순위상관 0.69, 거울 개수 하나만 쓰면 0.86 이었다.
   사람은 솔버처럼 경로를 펼치는 게 아니라 놓을 조각 수만큼 손을 움직인다.

   그래서 거울 개수를 주 지표로 올리고 노드 수는 보조로 내렸다.

   ⚠ 이 가중치는 잠정이다. 표본이 8판뿐이고, 더 큰 문제가 따로 있다 — 어려운 사양일수록
   해가 수천 개다(8×8 거울8 은 중앙값 3000+). 해가 그렇게 많으면 아무렇게나 놓아도
   맞으므로, 이 점수가 재고 있는 건 어려움이 아니라 손이 가는 정도다. 생성기가 제약이
   빡빡한 판을 만들도록 고친 뒤에 이 식을 다시 맞춰야 한다.

   절대값에는 의미가 없다 — 판들을 줄 세우는 데만 쓴다. */
export function difficulty({ nodes, solutions, mirrors }) {
  const search = Math.log2(nodes + 1) * 4;
  const scarcity = 12 / Math.min(Math.max(solutions, 1), 12) * 6;
  return Math.round(search + mirrors * 15 + scarcity);
}
